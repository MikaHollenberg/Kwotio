...
export async function loadQuoteBlocks(supabase: Client, quoteId: string): Promise<BlockDraft[]> {
  ...
  for (const row of blockRows ?? []) {
    ...
    const content = row.content as { heading?: string; intro?: string };
    const resolvedContent = { heading: content.heading ?? "", intro: content.intro ?? "", packages, addons };
    blocks.push({
      id: row.id,
      type: "packages",
      position: row.position,
      ...
      const { error: blockError } = await supabase.from("quote_blocks").upsert(
        {
          ...
          type: "packages",
          position: i,
          content: { heading: content.heading, intro: content.intro },
        },
        { onConflict: "id" },
      );
      if (blockError) throw blockError;
