...
import { updateOrganizationSettings, updateOrganizationLogo, updateLogoPreference, updateOrganizationTerms, type OrganizationSettingsFields } from "./actions";
...
/** Bedrijfsgegevens-categorie op Instellingen: naam, contactgegevens, adres,
 * logo's, huisstijlkleuren en algemene voorwaarden. De publieke-
 * offertepagina-specifieke velden (link, welkomsttekst, achtergrondstijl,
 * locatiefoto, aantal-personen-veld) zitten sinds het Instellingen-
 * herontwerp in de losse `PublicPageSettingsCard`, en het review-linkje in
 * `EmailAutomationCard` (het voedt alleen de review-aanvraag-e-mail). */
export function OrganizationSettingsCard({
      ...
      <div className="grid gap-6 sm:grid-cols-[auto_1fr]">
        <div className="flex items-center justify-center rounded-brand-sm bg-sand-200 p-6">
          {(logoPreference === "vierkant" ? logoSquareUrl : logoHorizontalUrl) ? (
            <Logo logoUrl={logoPreference === "vierkant" ? logoSquareUrl : logoHorizontalUrl} height={72} />
          ) : (
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <form
        className="flex flex-col gap-6 border-t border-ink-100 pt-4"
        onSubmit={(e) => {
          e.preventDefault();
          setError(null);
