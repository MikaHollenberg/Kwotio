    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex flex-col gap-2">
          ...
          <div className="kw-skeleton h-7 w-36 rounded-brand-sm" />
        </div>
        <div className="kw-skeleton h-9 w-36 rounded-brand-sm" />
      </div>
      <div className="kw-skeleton h-9 w-64 rounded-brand-sm" />
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="kw-skeleton h-32 w-full rounded-brand-lg" />
        ))}
      </div>
    </div>
  );
