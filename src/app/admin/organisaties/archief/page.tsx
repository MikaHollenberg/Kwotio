...
export default async function AdminOrganisatiesArchiefPage() {
  const organizations = await getOrganizationsList({ archived: true });
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <h2 className="font-display text-2xl font-semibold text-ink-500">Archief</h2>
        </div>
        <ButtonLink href="/admin/organisaties" variant="outline">
          <ArrowLeft className="size-4" /> Terug naar organisaties
        </ButtonLink>
      </div>
      ...
      <Card>
        <CardContent className="p-0">
          {organizations.length === 0 ? (
            <p className="p-6 text-sm text-ink-400">Geen gearchiveerde organisaties.</p>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-ink-100 text-left text-xs font-semibold uppercase tracking-wide text-ink-400">
                    ...
                    <th className="px-4 py-3 text-right">Gearchiveerd op</th>
                  ...
                  {organizations.map((org) => (
                    <tr key={org.id} className="border-b border-ink-50 last:border-0 hover:bg-sand-100">
                      ...
                      <td className="px-4 py-3 text-right text-ink-400">
                        {org.archivedAt ? formatDate(org.archivedAt) : "—"}
                      </td>
                      <td className="px-4 py-3">
                        <OrganizationRowActions organizationId={org.id} name={org.name} archived />
                      </td>
                    </tr>
                  ))}
                </tbody>
